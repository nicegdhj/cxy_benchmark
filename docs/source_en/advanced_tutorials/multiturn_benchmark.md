...
## Quick Start
### Usage Notes
+ ⚠️ Only evaluation for the `/v1/chat/completions` API is supported; you must specify `--models vllm_api_stream_chat_multiturn`.
...
### Command Explanation
Take the performance evaluation scenario of **ShareGPT multi-turn dialogue on the vLLM service backend** as an example:
```bash
ais_bench --models vllm_api_stream_chat_multiturn --datasets sharegpt_gen --debug -m perf
```

Where:
- `--models`: Specifies the model task, i.e., the `vllm_api_stream_chat_multiturn` model task.
...
- The model configuration file `vllm_api_stream_chat_multiturn.py` contains settings related to model operation and **must be modified according to your actual environment**. Critical fields to modify are annotated in the code below:
  ```python
  ...
  models = [
      dict(
          ...
          custom_client=dict(type=OpenAIChatStreamClient),  # Set to OpenAIChatStreamSglangClient when using the SGLang service
...
└── performance           # Performance evaluation results
     └── vllm-multiturn-api-chat-stream/          # Name of the "service-based model configuration" (corresponds to the `abbr` parameter in the model task configuration file)
