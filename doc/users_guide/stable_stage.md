...
### 与常规性能测试的区别
稳态测试与📚 [常规服务化性能测试](performance_benchmark.md)区别在于仅在于**性能数据的计算方式**不同：
```mermaid
graph LR;
...
![图2](../../img/stable_stage/concurrency_plot.png)
...
## 稳态测试快速入门
### 命令含义
稳态测试与📚 [服务化性能测评快速入门/命令含义](.performance_benchmark#命令含义)中的解释相同。在此基础上需要指定`--summarizer stable_stage`按照稳态方式统计性能数据，，以如下AISBench命令为例：
```shell
...
## 主要功能场景
服务化性能压力测试是常规服务化性能测试的拓展，因此也支持📚 [常规性能测试的主要功能场景](./performance_benchmark.md#主要功能场景)。想要进行稳态测试，只需要在原来性能测试的基础上指定`--summarizer stable_stage`
### 单任务评测
...
![图1](../../img/stable_stage/pressure_text_plot.png)
### 压力测试快速入门
