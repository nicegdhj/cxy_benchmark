    ...
    def _score(self):
        ...
        if not osp.exists(osp.realpath(filename)) and not osp.exists(
            ...
            if 'pred_postprocessor' in self.model_cfg:
                ...
                self.logger.info("----------------Model pred postprocessor------------------------")
                self.logger.info(f"Model pred postprocessor: {self.model_cfg['pred_postprocessor']}")
                ...
                for i in range(min(3, len(pred_strs))):
                    ...
                    ref = test_set[self.output_column][i] if self.output_column and i < len(test_set[self.output_column]) else 'N/A'
                    self.logger.info(f"[Sample {i}] Processed Pred: {repr(proc_pred)} | Ref: {repr(ref)}")
            # Postprocess predictions if necessary
            if 'pred_postprocessor' in self.eval_cfg:
                self.logger.debug(f"Eval pred postprocessor: {self.eval_cfg['pred_postprocessor']}")
                self.logger.info("----------------Eval pred postprocessor------------------------")
                self.logger.info(f"Eval pred postprocessor: {self.eval_cfg['pred_postprocessor']}")
                ...
                if pred_list_flag:
                    ...
                    pred_strs = [proc(s, **kwargs) for s in pred_strs]
                self.logger.info("=== DEBUG: After eval-level postprocessor (FINAL PREDICTIONS) ===")
                for i in range(min(3, len(pred_strs))):
                    final_pred = pred_strs[i]
                    ref = test_set[self.output_column][i] if self.output_column and i < len(test_set[self.output_column]) else 'N/A'
                    match = final_pred == ref
                    self.logger.info(f"[Sample {i}] Final Pred: {repr(final_pred)} | Ref: {repr(ref)} | Match: {match}")
            model_pred_strs = []
            if 'model_postprocessor' in self.eval_cfg:
                ...
                self.logger.info("=== DEBUG: After model_postprocessor (for reference only) ===")
                refs = test_set[self.output_column] if self.output_column else ['N/A'] * len(model_pred_strs)
                for i in range(min(2, len(model_pred_strs))):
                    ...
                    self.logger.info(f"[Sample {i}] ModelPost Pred: {repr(mp_pred)} | Ref: {repr(ref)}")
