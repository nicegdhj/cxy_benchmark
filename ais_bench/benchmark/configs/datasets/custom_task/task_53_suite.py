...
SYSTEM_INSTRUCTION = """你是一位故障分类专家，擅于对投诉信息进行分类，并整理成JSON格式，具体格式为
```json
{
  "classification": ...,
...
8、主要内容为信号、网络等问题：
  - 没有更具体的说明问题情况，只有简略的诸如没信号、信号弱、网络不好、通话、呼叫有问题等，此时分类标号为“1”，结果为“语音基础业务-VOLTE”;
...
用户提示词：
投诉分类专家你好，我们现在有一个投诉信息，需要请你对其进行分类，整理成结构化JSON对象。请你对该信息进行分析推理，并输出最终结果。
...
task_53_reader_cfg = dict(
    input_columns=['input'],
    output_column='output',
)

task_53_infer_cfg = dict(
...
task_53_datasets = [
    dict(
        type=CustomDataset,
        abbr='task_53',
        path='data/custom_task/task_53.jsonl',
        reader_cfg=task_53_reader_cfg,
        infer_cfg=task_53_infer_cfg,
        eval_cfg=task_53_eval_cfg,
    )
]
