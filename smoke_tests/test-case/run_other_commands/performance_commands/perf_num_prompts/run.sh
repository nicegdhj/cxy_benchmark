...
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} --mode perf --debug --num-prompts 10 --work-dir ${CASE_OUTPUT_PATH} 2>&1 | tee ${CUR_DIR}/tmplog.txt
if [ $? -eq 0 ]
then
...
# 获取实际请求数
total_requests=$(grep "Total Requests" ${CUR_DIR}/tmplog.txt | tail -n 1 | grep -o "[0-9]\+")
echo "total_requests ${total_requests}"
if [ "${total_requests}" != "10" ];then
    echo "Actual request not equal to set --num-prompt"
    exit $ret_failed
fi
