...
#custom dataset path
CUSTOM_DATASETS_PATH=$CUR_DIR/test_mcq.jsonl
...
ais_bench --models ${CASE_NAME} --custom-dataset-path ${CUSTOM_DATASETS_PATH} --custom-dataset-data-type mcq --work-dir ${CASE_OUTPUT_PATH} 2>&1 | tee ${CUR_DIR}/tmplog.txt
if [ $? -eq 0 ]
then
