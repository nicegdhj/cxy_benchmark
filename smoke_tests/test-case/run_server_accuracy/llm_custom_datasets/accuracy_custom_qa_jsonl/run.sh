...
# CONFIG_DATASET_NAME="SuperGLUE_BoolQ"
OUTPUT_DATASET_NAME="test_qa"
CURR_API="vllm-api-general-chat"

#custom dataset path
CUSTOM_DATASETS_PATH=$CUR_DIR/test_qa.jsonl
...
ais_bench --models ${CASE_NAME} --custom-dataset-path ${CUSTOM_DATASETS_PATH} --custom-dataset-data-type qa --work-dir ${CASE_OUTPUT_PATH} 2>&1 | tee ${CUR_DIR}/tmplog.txt
if [ $? -eq 0 ]
then
