...
ais_bench --models ${CASE_NAME} --datasets ${CASE_NAME} --work-dir ${CASE_OUTPUT_PATH} --max-num-workers 10 2>&1 | tee ${CUR_DIR}/tmplog.txt
if [ $? -eq 0 ]
then
