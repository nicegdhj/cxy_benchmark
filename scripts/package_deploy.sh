...
#
# 功能：
#   1. 将 Docker 镜像导出为 .tar.gz（如果 outputs/ 下还没有）
#   2. 将 data/ + .env + run_mixed_benchmark.sh + 镜像 按 deploy.md 目录结构打包
...
#   - 项目根目录下有 data/、.env、run_mixed_benchmark.sh
# ==============================================================================
...
# 镜像文件用硬链接或符号链接（避免二次拷贝，节省时间）
echo "  链接镜像文件 $IMAGE_FILE..."
...
echo "  3. cd eval_workspace && docker load < benchmark-eval.tar.gz"
echo "  4. bash run_mixed_benchmark.sh"
echo "======================================================"
